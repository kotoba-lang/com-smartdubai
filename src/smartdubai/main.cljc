(ns smartdubai.main
  "Kotodama WASM entrypoint for the Smartdubai clean-room actor (L4) — Clojure port.
  ...
  facts (`smartdubai.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "smartdubai")
(def tier "L4")
...
(def entity-specs
  [{:entity "Identity"   :plural "identities"   :id-prefix "smartdub_ide"
    :fields [:externalId :jurisdiction :verified]              :required [:jurisdiction :verified]
    :coerce {:verified :bool}                       :refs {}}
   {:entity "Credential"      :plural "credentials"      :id-prefix "smartdub_cre"
    :fields [:identityId :type :issuer :issuedAt] :required [:type :issuer]
    :coerce {}          :refs {:identityId "Identity"}}
   {:entity "Consent"     :plural "consents"     :id-prefix "smartdub_con"
    :fields [:identityId :purpose :granted :expiresAt]        :required [:purpose :granted]
    :coerce {:granted :bool} :refs {:identityId "Identity"}}
   {:entity "Transaction" :plural "transactions" :id-prefix "smartdub_tra"
    :fields [:identityId :type :amount :status]          :required [:type :amount]
    :coerce {:amount :float}          :refs {:identityId "Identity"}}
   {:entity "Document" :plural "documents" :id-prefix "smartdub_doc"
    :fields [:identityId :type :contentRef]          :required [:type]
    :coerce {}                       :refs {:identityId "Identity"}}
   {:entity "VerificationRequest"   :plural "verificationrequests"   :id-prefix "smartdub_ver"
    :fields [:identityId :verifier :status]   :required [:verifier :status]
    :coerce {}        :refs {:identityId "Identity"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"smartdubai.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "smartdubai-compat" :tier tier :entities entities} 200])
...
(defn start! [] :smartdubai-compat/ready)
